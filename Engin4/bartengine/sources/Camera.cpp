...
bool bart::Camera::CollideWith(const Rectangle& aRect) const
{
    return Engine::Instance().GetCollision().IsColliding(mViewport, aRect);
}

bool bart::Camera::CollideWith(const Circle& aCircle) const
{
    return Engine::Instance().GetCollision().IsColliding(mViewport, aCircle);
}

bool bart::Camera::CollideWidth(const int aX, const int aY) const
{
